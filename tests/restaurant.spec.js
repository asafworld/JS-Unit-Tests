...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const novoObjetoRetornado = createMenu();
    novoObjetoRetornado.order('coxinha');
    novoObjetoRetornado.order('agua');
    novoObjetoRetornado.order('coxinha');
    expect(novoObjetoRetornado.consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    ...
    // ```
    const finalMenu = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
    finalMenu.order('coxinha');
    finalMenu.order('agua');
    finalMenu.order('coxinha');
    finalMenu.pay() // Retorno: somaDosPreçosDosPedidos
    expect(finalMenu.pay()).toBe(11.7)
